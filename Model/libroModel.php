...
class LibroModel extends Conectar{
	...
	public function agregar_libro($cNombre, $codigo, $nAutor_fk, $nPrograma_fk  ,$lestado){
		$query_save = "Insert into tadmin(cNombre,codigo, nAutor_fk, nPrograma_fk,lestado) value(:cNombre,:codigo, :nAutor_fk, :nPrograma_fk ,:lestado)";
        $guardar = $this->conexion->prepare($query_save);
        $guardar->bindParam(':cNombre', $datos['cNombre']);
        $guardar->bindParam(':codigo', $datos['codigo']);
        $guardar->bindParam(':nAutor_fk', $datos['nAutor_fk']);
        $guardar->bindParam(':nPrograma_fk', $datos['nPrograma_fk']);
